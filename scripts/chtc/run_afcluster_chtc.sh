...
if [ -d "/staging/$(USER)/af2_weights" ] && [ "$(ls -A /staging/$(USER)/af2_weights 2>/dev/null)" ]; then
    echo "Attempting to copy weights from staging..."
    if cp /staging/$(USER)/af2_weights/* ./cache/colabfold/params/ 2>/dev/null && [ "$(ls -A ./cache/colabfold/params/ 2>/dev/null)" ]; then
